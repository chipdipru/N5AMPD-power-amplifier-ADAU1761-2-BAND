...
void Init()
{
  ...
  UserPOTs[0].ParamData = (uint8_t*)ADAU1761_OUT_VOLUME;//ADAU_VOLUME;
  UserPOTs[1].ParamData = (uint8_t*)ADAU1761_OUT_VOLUME;//ADAU_VOLUME;
  
  UserPOTs[0].ParamAdr = ADAU1761_LOUTVOL_ADR;//ADAU1761_LEFT_VOL_ADR;
  UserPOTs[1].ParamAdr = ADAU1761_LOUTVOL_ADR;//ADAU1761_LEFT_VOL_ADR;
  
  UserPOTs[2].ADCStep = ADC_STEP_PER_VOLUME_UNIT;
  UserPOTs[2].ParamData = (uint8_t*)ADAU1761_HP_VOLUME;
  UserPOTs[2].ParamAdr = ADAU1761_LHPVOL_ADR;
  UserPOTs[2].Response = ADAU1761_ChangeHPVolume;
  
  UserPOTs[3].ParamAdr = ADAU1761_EQ120Hz_START_ADR;
  UserPOTs[4].ParamAdr = ADAU1761_EQ5000Hz_START_ADR;
  UserPOTs[3].ParamData = (uint8_t*)ADAU_EQ_SHELF_120Hz;
  UserPOTs[4].ParamData = (uint8_t*)ADAU_EQ_SHELF_5000Hz;
  
  for (uint8_t i = 3; i < ADC_CHANNELS; i++)
  {
    UserPOTs[i].ADCStep = 185;//ADC_STEP_PER_EQ_UNIT;
    UserPOTs[i].Response = DSP_ChangeEQParam;
  }  
...
void TAS3251_SwapFlag()
{
  ...
  InitData = TAS3251_DSP_BOOK;
  DSP_Write(TAS3251_CHANGE_BOOK_REG, 1, &InitData);
  
  InitData = TAS3251_DSP_SWAP_FLAG_PAGE;
  DSP_Write(TAS3251_CHANGE_PAGE_REG, 1, &InitData);
  
  DSP_Write(TAS3251_DSP_SWAP_REG, TAS3251_DSP_SWAP_SIZE, (uint8_t *)TAS3251_DSP_SWAP_DATA);
  
  //возврат на страницу регулировки
  InitData = 0;
  DSP_Write(TAS3251_CHANGE_PAGE_REG, 1, &InitData);
  DSP_Write(TAS3251_CHANGE_BOOK_REG, 1, &InitData);
}
...
void DSP_Init()
{
  ...
  uint8_t HPMuteVal = ADAU1761_HP_VOLUME_MUTE;
  DSP_Write(ADAU1761_LHPVOL_ADR, 1, &HPMuteVal);
  DSP_Write(ADAU1761_RHPVOL_ADR, 1, &HPMuteVal);
  DSP_Write(ADAU1761_LOUTVOL_ADR, 1, &HPMuteVal);
  DSP_Write(ADAU1761_ROUTVOL_ADR, 1, &HPMuteVal);
  
  //SW Vol 0 dB
  ADAU1761_SafeLoad(ADAU1761_LEFT_VOL_ADR, (uint8_t*)(&ADAU_VOLUME[ADAU_VOLUME_0dB_INDEX]), ADAU1761_SAFELOAD_NUMLOAD_1);
  ADAU1761_SafeLoad(ADAU1761_LEFT_VOL_STEP_ADR, (uint8_t*)ADAU_VOLUME_STEP, ADAU1761_SAFELOAD_NUMLOAD_1);
  ADAU1761_SafeLoad(ADAU1761_RIGHT_VOL_ADR, (uint8_t*)(&ADAU_VOLUME[ADAU_VOLUME_0dB_INDEX]), ADAU1761_SAFELOAD_NUMLOAD_1);
...
*/
  ...
  DSP_Write(ADAU1761_LOUTVOL_ADR, 1, (uint8_t*)(&ADAU1761_OUT_VOLUME[LeftVolume]));
  DSP_Write(ADAU1761_ROUTVOL_ADR, 1, (uint8_t*)(&ADAU1761_OUT_VOLUME[RightVolume]));
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
void ADAU1761_ChangeHPVolume(uint8_t POTNum, uint16_t ADCValue)
{
  uint8_t VolIndex = ADCValue / ADC_STEP_PER_VOLUME_UNIT;
  
  DSP_Write(ADAU1761_LHPVOL_ADR, 1, (uint8_t*)(&ADAU1761_HP_VOLUME[VolIndex]));
  DSP_Write(ADAU1761_RHPVOL_ADR, 1, (uint8_t*)(&ADAU1761_HP_VOLUME[VolIndex]));
}
//------------------------------------------------------------------------------
...
void DSP_Mute(uint8_t Flag)
{
  ...
  else
  {
    ...
    //HP
    ADAU1761_ChangeHPVolume(9, ADC_GetValue(9));
  }
